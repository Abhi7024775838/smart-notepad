...
class Notepad:
    def __init__(self):
        ...
        print("controller is set, db status is checked")
        self.root.protocol("VM_DELETE_WINDOW", self.exit_func)
    ...
    def set_menu_bar(self):
        ...
        self.color_dict = {'Light Default': ('#000000', '#ffffff'),
    ...
    def set_tool_bar(self):
        ...
        print("set tool bar ends")

    def set_tool_bar_event_bindings(self):
        # font family and font size functionality
        self.current_font_family = 'Ariel'
        ...
        try:
            ...
            if self.db_controller.is_secure_file(self.base):
                ...
                if pwd == self.db_controller.get_file_pwd(self.base):
                    ...
                    messagebox.showerror("wrong pass", "pass invalid!")
            else:
                self.text_editor.delete(1.0, tk.END)
                self.text_editor.insert(1.0, self.msg)
                self.root.title(self.base)
        except FileNotFoundError:
            messagebox.showerror("wrong file name ", "file does not exists!")
        except:
            print(traceback.format_exc())
    ...
    def say_something(self):
        try:
            ...
            if self.takeAudio == "":
                messagebox.showinfo("say again", "speech not recoganized!")
            ...
            messagebox.showerror("No internet", "please check yur internet connection")
        except WaitTimeoutError as wte:
            messagebox.showerror("Time over", 'time limit exceeded')
        except UnknownValueError as unkwe:
            messagebox.showerror("non translatable text", 'Speech unrecogizable!')

    def align_left(self):
        text_content = self.text_editor.get(1.0, 'end')
        self.text_editor.tag_config('left', justify=tk.LEFT)
        self.text_editor.delete(1.0, tk.END)
        self.text_editor.insert(tk.INSERT, text_content,
                                'left')  # tk.INSERT tells the current position of the text_editor
    ...
    def change_font_color(self):
        ...
        self.text_editor.configure(color_var[1])
    ...
    def secure_file(self):
        if self.db_status:
            ...
            self.file_open_button.grid(row=2, column=1, padx=4, pady=4)
            self.file_remove_button.grid(row=2, column=2, padx=4, pady=4)
    ...
    def add_file(self):
        try:
            if self.file_owner_input.get().strip() != " " and self.file_pwd_input.get().strip() != "":
                ...
                messagebox.showerror("Error!", " plz enter file owner and pass")
        except:
            messagebox.showerror("db error", "error while adding file")
            print("here in add file error occured")
            print(traceback.format_exc())
    ...
    def open_secure_file(self):
        try:
            ...
            if len(self.sel_file_index_tuple) == 0:
                messagebox.showerror("error", "Please sel a file first")
            else:
    ...
    def run(self):
        print("yes its(run) working")
        self.root.mainloop()

        # self.find_dialoge-tk.Topleveree()
        # self.find_frame=ttk.L
