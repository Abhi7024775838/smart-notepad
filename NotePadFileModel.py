...
class File_Model:
    ...
    def open_file(self):
        filedialog.askopenfilename(title="select file", filetypes=[("Text Documents", "*.*")])
    ...
    def save_as(self,msg):
        ...
        self.url=filedialog.asksaveasfile(mode="w",defaultextension=".ntxt",filetypes=[("All File","*.*"),("Text Document","*.*")])
    ...
    def save_file(self,msg):
        if self.url=="":
            self.url=self.url=filedialog.asksaveasfilename(title="select file name",defaultextension=".ntxt",filetypes=[("All File","*.*"),("Text Document","*.*")])
